...
openai_client = OpenAI(
    ...
    organization='org-PJgt1G5gjj3OUC2fjRGWqZku',
    )
...
async def start_chat():
    ...
    cl.user_session.set(
        ...
        [
            {
                "role": "system",
                "content": "Dein Name ist Dinesh und du bist humorvoll und machst gelegentliche witze, als Onboarding-Assistent und technisches Genie bei MetallicaTech GmbH, ist es deine Aufgabe, neue Teammitglieder durch ihren Einstieg zu führen. Basierend auf dem folgenden Kontext:{context} Frage: {question}. Antworte präzise und vermeide lange Texte",
            }
        ],
    )
    await cl.Avatar(
...
async def prompt_for_user_name():
    try:
        response = await cl.AskUserMessage(content="Hallo, ich bin Dinesh, dein Onboarding-Assistent. Wie heißt du?",
                                            author="Dinesh",
                                            timeout=30
        ).send()
...
async def answer_as(name, message: cl.Message):
    ...
    # Prüfen, ob runnable vorhanden ist und Nachrichten streamen
    if runnable:
        async for chunk in runnable.astream(
            message.content,
            config=RunnableConfig(callbacks=[cl.LangchainCallbackHandler()]),
        ):
            if chunk:  # Sicherstellen, dass chunk ein nicht-leerer String ist
                await msg.stream_token(chunk)

    await msg.update()

    # Sicherstellen, dass die Nachrichtenhistorie eine Liste von Dictionaries ist
    history = [{"role": msg["role"], "content": msg["content"]} for msg in message_history]

    stream = openai_client.chat.completions.create(
        model=model_name,
        messages=history + [{"role": "user", "content": f"speak as {name}"}],
        stream=True,
        **settings,
    )
